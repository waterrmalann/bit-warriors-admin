...
export default function DashboardEditProblemPage() {
  ...
  useEffect(() => {
    if (testCasesData && firstProblemFetch.current) {
      setTestCases(JSON.stringify(testCasesData.testCases, null, 2));
      setFunctionName(testCasesData.functionName);
      setLanguage(testCasesData.language);
      setPreloadedCode(testCasesData.preloadedCode || '');
      firstProblemFetch.current = false;
    }
  }, [testCasesData])
